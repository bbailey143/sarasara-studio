(function(global){
  ...
  const PROFILES={
    ...
    charcoal:{
      id:'material.charcoal.diagnostic.v0.5',version:'0.5.0',
      ...
      models:['MODEL-DEPO-001','MODEL-PART-001','MODEL-TRIB-001'],interactions:['IM-008','IM-009'],provenance:[{status:'stand-in',note:'Artist-recognizable diagnostic profile. Fracture toughness, abrasion resistance, coarse/fine shares, density, shape, and settling are normalized unmeasured stand-ins requiring artist calibration.'}]
    }
  };
  ...
  class SharedSolver{
    ...
    addDisk(cx,cy,radius,water,pigment,pressure,speed,brushMoisture,strokeX=0,strokeY=0){
      ...
      for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){
        ...
        if(dry){const tooth=this.tooth(x,y),contact=tooth*.72+pressure*.48,potential=pigment*k;this.sourceOfferedPigment+=potential;if(contact<.34||(speed>1.05&&contact<.58&&((x+y)%3===0))){this.sourceRemainingPigment+=potential;continue}const amount=Math.min(potential,potential*(.35+rough*tooth)),split=this.fractureSplit(pressure,speed,tooth),coarseMass=amount*split.coarse,fineMass=amount*split.fine,settledMass=Math.max(0,amount-coarseMass-fineMass),side=this.noise(x,y,this.substrate.texture.seed+997)*2-1,normalX=-strokeY,normalY=strokeX,coarseSpeed=.7+pressure*.85+speed*.45,fineSpeed=2.2+pressure*1.7+speed*2.7;this.sourceRemainingPigment+=potential-amount;this.deposited[i]+=settledMass;this.addParticlePopulation(coarseMass,i,strokeX*coarseSpeed+normalX*side*.35,strokeY*coarseSpeed+normalY*side*.35,this.coarse,this.coarseVx,this.coarseVy);this.addParticlePopulation(fineMass,i,strokeX*fineSpeed+normalX*side*1.4,strokeY*fineSpeed+normalY*side*1.4,this.fineDust,this.fineDustVx,this.fineDustVy);this.coarseCreatedPigment+=coarseMass;this.fineCreatedPigment+=fineMass;addedPigment+=amount}
        else{
    ...
    stepDetachedPopulation(dt,population,vxField,vyField,nextPopulation,nextMx,nextMy,kind){
      const w=this.w,h=this.h,roughness=this.s['SUBI-001'],packing=Math.max(0,Math.min(1,this.p['DEPO-004'])),friction=this.p['TRIB-002'],coarse=kind==='coarse',drag=Math.exp(-(coarse?2.6+friction*2.1+roughness*1.3:.55+friction*.7+roughness*.35)*dt),settleRate=coarse?1.7+packing*.8+roughness*.8:.12+packing*.18+roughness*.1,settleCap=coarse?.3:.055;nextPopulation.fill(0);nextMx.fill(0);nextMy.fill(0);
