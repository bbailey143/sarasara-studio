(function(global){
  ...
  const SUBSTRATES={
    ...
    fineTooth:{id:'substrate.paper.fine-tooth-drawing.experimental.v0.1',name:'Fine-Tooth Drawing Paper',version:'0.1.0',state:{'TRAN-002':.018,'SUBI-001':.22,'SUBI-003':.32},texture:{tooth:.22,absorbency:.25,sizing:.75,capacity:.32,dryBrushBreakup:.35,seed:314,noiseScale:.35,paperColor:'#f8f5ee'},provenance:[{status:'stand-in',note:'Artist-directed drawing-paper candidate with smaller, shallower tooth; requires artist validation and later measurement.'}]}
  };
  ...
  class SharedSolver{
    ...
    render(target,canvas){
      ...
      for(let i=0;i<this.n;i++){
        ...
        const color=this.substrate.texture.paperColor,base=parseInt(color.slice(1),16),roughness=this.s['SUBI-001'],shade=(paperNoise-.5)*(5+roughness*22);let pr=(base>>16&255)+shade,pg=(base>>8&255)+shade,pb=(base&255)+shade;
