(function(global){
  ...
  class SharedSolver{
    ...
    flowLayer(dt){
      ...
      const packing=Math.max(.05,Math.min(1,Number(this.p['DEPO-004'])||.05)),grainDensity=Math.max(.05,Math.min(1,Number(this.p['PART-002'])||.05)),reliefDivisor=packing*grainDensity;
      const slump=(i,j)=>{if(DP[i]<=0)return;const drop=DP[i]/reliefDivisor-DP[j]/reliefDivisor;if(drop<=0)return;const excess=drop-yieldStress;if(excess<=0)return;const flux=Math.min(DP[i]*.2,excess*dt*.5/viscosity);if(flux<=0)return;ND[i]-=flux;ND[j]+=flux;moved+=flux};
    ...
    addParticlePopulation(mass,index,vx,vy,population,vxField,vyField){if(mass<=0)return;this.particlesLive=true;const old=population[index],combined=old+mass;vxField[index]=(vxField[index]*old+vx*mass)/combined;vyField[index]=(vyField[index]*old+vy*mass)/combined;population[index]=combined}
    ...
    buildPaperSurface(){this.paperHeight=new Float32Array(this.n);this.paperVisual=new Float32Array(this.n);for(let y=0;y<this.h;y++)for(let x=0;x<this.w;x++){const i=y*this.w+x,sample=this.samplePaperSurface(x,y);this.paperHeight[i]=sample.height;this.paperVisual[i]=sample.visual}this.bakePaperColour()}
    ...
    addDisk(cx,cy,radius,water,pigment,pressure,speed,brushMoisture,strokeX=0,strokeY=0,sweep=1,shape=null){
      ...
      for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){
        ...
        if(body){
          ...
          budget-=amount;
          this.wetLive=true;this.water[i]=Math.min(2.5,this.water[i]+water*k);this.mobile[i]+=mobileAmount;this.deposited[i]+=amount-mobileAmount;addedPigment+=amount;
        }
      }
    ...
    smudgeSegment(ax,ay,bx,by,canvasW,canvasH,pressure,speed){
      ...
      this.deposited.set(next);this.particlesLive=true;this.loose.set(nextLoose);this.looseVx.set(nextLooseVx);this.looseVy.set(nextLooseVy);this.relocatedPigment+=relocated;return relocated;
    }
    ...
    step(dt){
      this.elapsed+=dt;this.flowLayer(dt);
      /* Crumbs and dust move only if any exist. The three passes below each
         blank three whole grids before they start, so running them over an
         empty field is not free - it was costing oil and watercolour, which
         never make a particle, about a sixth of every frame. */
      if(this.particlesLive){
        this.stepSurfaceParticles(dt);
        this.stepDetachedPopulation(dt,this.coarse,this.coarseVx,this.coarseVy,this.nextCoarse,this.nextCoarseMx,this.nextCoarseMy,'coarse');
        this.stepDetachedPopulation(dt,this.fineDust,this.fineDustVx,this.fineDustVy,this.nextFineDust,this.nextFineDustMx,this.nextFineDustMy,'fine');
      }
      const wet=this.p['COMP-001']>.02;if(!wet)return;
      /* And water moves only if there is water. A material can be thin enough
         to flow and still have a bone-dry sheet in front of it - oil never wets
         the sheet at all, so this whole solve was running on a field of zeroes
         every frame of every oil painting. */
      if(!this.wetLive)return;
      ...
      NW.set(W);NM.set(M);let stillWet=false;
      ...
      for(let i=0;i<this.n;i++){
        ...
        if(water>1e-7||mobile>1e-7||AB[i]>1e-7)stillWet=true;
      }
      this.wetLive=stillWet;
     ...
     * Mix the paper once.
     *
     * Colour, grain and fibre shading depend on the sheet and the substrate and
     * on nothing else - they cannot change while you paint. They were being
     * worked out again for all 106,400 cells on every frame, including parsing
     * the same paper colour out of the same string 106,400 times a frame, which
     * cost more than the paint physics did. Now the paper is mixed when the
     * sheet or the surface changes and the frame just lays paint over it.
     */
    bakePaperColour(){
      const texture=this.substrate.texture,base=parseInt(String(texture.paperColor).slice(1),16);
      const r0=base>>16&255,g0=base>>8&255,b0=base&255;
      const grain=5+this.s['SUBI-001']*22,contrast=texture.visualFiberContrast||0;
      const shade=this.paperShade=new Float64Array(this.n),data=this.image.data;
      for(let i=0;i<this.n;i++){
        shade[i]=(this.paperHeight[i]-.5)*grain-this.paperVisual[i]*contrast;
        data[i*4+3]=255;   // opaque for good; the frame loop never touches alpha again
      }
      this.paperInk=[r0,g0,b0];
    }
    render(target,canvas){
      const data=this.image.data,shade=this.paperShade,w=this.w;
      const inkR=this.paperInk[0],inkG=this.paperInk[1],inkB=this.paperInk[2];
      const renderRegime=this.regime(),body=renderRegime==='body',dry=renderRegime==='granular';
      /* Everything the same for every pixel, worked out once instead of per cell.
         reliefHeight() in particular was three calls deep in the body path, each
         re-reading two properties and clamping them, ~320,000 times a frame. */
      const clamp01=v=>Math.max(0,Math.min(1,v));
      const reliefDivisor=Math.max(.05,Math.min(1,Number(this.p['DEPO-004'])||.05))*Math.max(.05,Math.min(1,Number(this.p['PART-002'])||.05));
      const packing=clamp01(this.p['DEPO-004']),density=clamp01(Number(this.p['PART-002'])||0);
      const dryDeposit=.72+packing*.7,dryCoarse=.78+density*.45,gain=this.displayGain;
      const D=this.deposited,M=this.mobile,L=this.loose,C=this.coarse,F=this.fineDust,W=this.water;
      for(let y=0,i=0;y<this.h;y++)for(let x=0;x<w;x++,i++){
        const j=i*4,tint=shade[i],pr=inkR+tint,pg=inkG+tint,pb=inkB+tint;
        if(body){
          const deposit=D[i],relief=deposit/reliefDivisor;
          const left=x>0?D[i-1]/reliefDivisor:relief,up=y>0?D[i-w]/reliefDivisor:relief;
          const lit=Math.max(-.6,Math.min(.6,((relief-left)+(relief-up))*1.2)),alpha=1-Math.exp(-deposit*5.4);
          data[j]=pr*(1-alpha)+Math.max(0,Math.min(255,118+lit*96))*alpha;
          data[j+1]=pg*(1-alpha)+Math.max(0,Math.min(255,76+lit*82))*alpha;
          data[j+2]=pb*(1-alpha)+Math.max(0,Math.min(255,52+lit*70))*alpha;
        }
        else if(dry){
          const opticalPigment=D[i]*dryDeposit+L[i]*.58+C[i]*dryCoarse+F[i]*.42,alpha=1-Math.exp(-opticalPigment*4.2);
          data[j]=pr*(1-alpha)+32*alpha;data[j+1]=pg*(1-alpha)+28*alpha;data[j+2]=pb*(1-alpha)+24*alpha;
        }
        else{
          const pigment=M[i]+D[i]+L[i]+C[i]+F[i];
          const alpha=Math.min(.74,1-Math.exp(-pigment*.78*gain)),wetGlow=Math.min(.045,W[i]*.018);
          data[j]=pr*(1-alpha)+44*alpha;data[j+1]=pg*(1-alpha)+105*alpha;data[j+2]=pb*(1-alpha)+158*alpha;
          data[j]=data[j]*(1-wetGlow)+220*wetGlow;data[j+1]=data[j+1]*(1-wetGlow)+236*wetGlow;data[j+2]=data[j+2]*(1-wetGlow)+245*wetGlow;
        }
      }
