...
| OL-P-12 | Drag-out | An empty brush pulled from thick paint carries colour onto bare ground and tapers. | `automated_relationship_verified` — after artist review OIL-002, the tool holds material it lifts. An empty brush dragged from a thick blob now advances the paint edge 21 cells (about six brush widths) where it previously advanced 0, tapering 0.726 to 0.020 to bare. Lifting is gated by yield stress, leaves the retained film, is proportional to the crossing travelled, and conserves across sheet and tool. Artist re-review pending. |
| OL-P-07 | Dirty brush | Painting into a wet layer picks material up onto the tool. | `partial_not_isolated` — the tool now genuinely holds material lifted off the sheet (OL-P-12), which is pickup. What is missing is releasing it into a *different* colour and any way to rinse, so the dirty-brush scene proper has not been run. |
